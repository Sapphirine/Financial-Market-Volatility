...
\rhead{Columbia University E6883 Big Data Analytics Fall 2014 Final Report}
...
\title{Financial Market Volatility }
...
\author{\IEEEauthorblockN{John Terzis, Tim Wu, Jimmy Zhong, Oliver Zhou }
\IEEEauthorblockA{Department of Electrical Engineering\\
Columbia University\\
jt2514@columbia.edu, tw2229@columbia.edu, jfz2107@columbia.edu, ohz2101@columbia.edu}
...
\begin{abstract}
Understanding volatility in financial markets has long been of interest to hedge and speculators. Empirical evidence has shown us that volatility is a highly nonlinear evolving process. 
Using various big data processing methods and algorithms, we seek to model various factors that affect volatility in the market
\end{abstract}
...
Modeling this process using the Hadoop ecosystem can offer tremendous advantages over traditional econometric models that are limited to datasets which fit in main memory.
...
Stuff I've referenced : to be cleaned up into paper format
http://algorithmicfinance.org/2-3-4/pp241-267/
Textbooks in class
...
as text files is joinfeatures.py. This routine walks through the raw data directory merging predesignated input data with each stock symbol’s data on the timestamp index and stores
...
return for these “displaced” closing prices versus the current timestamp. A resultant row from the reduce step looks like
...
the memory upwards that java has access to in the heap given the high memory usage in the reduce step.
 ...
 Add Volatility_HIGH as a cut off for VIX over .25
...
￼
Below is an example of a stock PEP’s transformed clustering matrix. Again the rows correspond to half hour intervals within a trading day and the columns
are 30 min return observations across the 20 year history of raw price data we have on this stock.


￼
The script Joinfeatures.py is meant to run as a python script on command line as “python joinfeatures.py” . Once resultant merged data is available, map reduce is ready to run using the helper script , vol_mapreduce.sh. You should see the hadoop streaming reporting incremental results of map-reduce as below.
...
over pyspark and hadoop. To run this script, set the parameters such as the input file URI and output file URI’s in constants.py within the package
and run using the command “/bin/pyspark svm.py” . 


\section{Experiment Results}
Describe the experiment results of your algorithm. Show how did you evaluate the performance of your algorithm.

Ridge Regression was run on AIG stock quotes between 2008 and 2001 from the transformed feature matrix generated by map reduce
